...
parser.add_argument('--net_type', default='HASHI_0mean_1std_YUV_mean_all_channels', type=str, help='model')
...
parser.add_argument('--num_epochs', default=25, type=int, help='Number of epochs in training')
...
# training_data_path = '/gpfs/scratch/hdle/breast_cancer_pos1'
...
#mean = [0.66954494, 0.530153, 0.58302003] # mean, std of YUV
#std = [0.17165746, 0.0259879, 0.04892175]

#mean = [0.6714873,  0.52777296, 0.60385185] # mean, std of YUV
#std = [0.17164655, 0.02275313, 0.06004456]

mean = [0.6714873,  0, 0] # mean, std of YUV
std = [0.17164655, 1, 1]
...
#mean_hashi, std_0, std_1 = get_mean_and_std_batch(train_set, bs = 4096)
#print('mean, std: ',mean_hashi, std_0, std_1)
...
def train_model(model, criterion = None, num_epochs=100, train_loader = train_loader, val_loader = val_loader):
    ...
    for epoch in range(num_epochs):
        ...
        # optimizer = optim.RMSprop(model.parameters(), lr=lr, weight_decay=args.weight_decay, momentum=0.9)
        ...
        for phase in ['train']:
            ...
            if (epoch + 1) % args.check_after == 0:
                ...
                # deep copy the model
                if val_auc > best_auc and epoch >= 0:
                    print('Saving model')
                    best_auc = val_auc
                    best_epoch = epoch
    ...
    print('Best val auc: {:4f} at epoch: {}'.format(best_auc, best_epoch))
