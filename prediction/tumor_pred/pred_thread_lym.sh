...
DATA_FILE=patch-level-paad.txt
    ...
    for files in ${FOLDER}/*/; do
        ...
        if [ -f ${files}/${DONE_FILE} ]; then
            if [ ! -f ${files}/${DATA_FILE} ]; then
                ...
                CUDA_VISIBLE_DEVICES=${LYM_CNN_PRED_DEVICE} python -u ${EXEC_FILE} ${files} ${LYM_NECRO_CNN_MODEL_PATH} ${DATA_FILE} ${LYM_NECRO_CNN_MODEL_PATH}/${MODEL}
            fi
        fi
    done
